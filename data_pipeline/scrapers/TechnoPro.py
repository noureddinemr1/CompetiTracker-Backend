...
class MytekScraper:
    def __init__(self):
        self.url = "https://www.technopro-online.com/"
    ...
    def scrape_page(self, page_url,category):
        ...
        for item in product_list.find_all('div', class_='js-product-miniature-wrapper col-12'):
            product_name = item.find('h2', class_='product-title')
            ...
            stock_status = item.find('div', class_='product-availability')
            ref = item.find('div', class_='product-reference')
            description = item.find('div' , class_="product-description-short")
            discount = item.find('li', class_='product-flag discount')
            image = item.find('img',class_="product-thumbnail-first")
            if price>=300 : 
                livraison = "Gratuite"
            else:
                livraison = "Payante"
            ...
            products.append({
                ...
                'category' : category,
                'stock_status': stock_status.text.strip().replace("\"\"", "") if ref else "Epuisé",
                'description': description.text.strip() if description else "N/A",
                'livraison' : livraison,
                'image': image.get("data-src") if image else "No Image",
